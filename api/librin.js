export default async function handler(req, res) {
  ...
  const intent =
    q.includes("simplif") || q.includes("fácil") || q.includes("facil")
      ? "simplify"
      : q.includes("idea") ||
          q.includes("central") ||
          q.includes("clave") ||
        ...
        : q.includes("3 punto") || q.includes("bullet") || q.includes("resum")
          ...
          : q.includes("ejemplo")
            ? "example"
            : q.includes("aplic") ||
                q.includes("vida real") ||
                q.includes("practic")
              ? "apply"
              : q.includes("crit") ||
                  q.includes("opini") ||
                  q.includes("qué piensas") ||
                  q.includes("que piensas")
                ? "opinion"
                : "chat";
      ...
      const resp = await fetch(
        "https://api.groq.com/openai/v1/chat/completions",
        {
          ...
          body: JSON.stringify({
            ...
            temperature:
              intent === "chat" || intent === "opinion" ? 0.85 : 0.65,
          }),
        },
      );
